		...
		$.ajax({
			...
			success: function(data, textStatus, jqXHR) {
				...
				for (var j = 0; j < parsedData.standings[0].table.length; j++) {
					...
					tableStructure.push('<tr><th scope="row">' + '<img height="4%" width="4%" src="' + currentTeam.team.crestUrl + '"></img> ' + currentTeam.team.name + '</th><td>' + currentTeam.playedGames + '</td><td>' + currentTeam.won + '</td><td>' + currentTeam.draw + '</td><td>' + currentTeam.lost + '</td><td>' + currentTeam.points + '</td></tr>');
				};
